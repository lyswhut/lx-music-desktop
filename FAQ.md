...
目前软件仅支持打开从网页粘贴过来的歌单，或者如果你知道歌单 id 也可以直接输入歌单 id 打开；<br>
事实上输入歌单链接时软件做的只是自动匹配出链接里的歌单 id 去打开，如果链接不包含歌单 id，将无法打开歌单；<br>
对于移动端分享出来的歌单，建议先在浏览器中打开后，再从浏览器复制歌单地址到软件打开；<br>
目前不支持打开酷狗用户自定义歌单。

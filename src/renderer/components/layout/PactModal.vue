  ...
  <material-modal :show="!isAgreePact || isShowPact" max-width="70%" :bg-close="isAgreePact" :close-btn="isAgreePact" @close="handleClose(false)">
    <main :class="$style.main">
      ...
      <div class="select scroll" :class="$style.content">
        ...
        <p>1.1&nbsp;本项目的各官方平台在线数据来源原理是从其公开服务器中拉取数据（与未登录状态在官方平台APP获取的数据相同），经过对数据简单地筛选与合并后进行展示，因此本项目不对数据的合法性、准确性负责。</p>
        ...
        <p>1.3&nbsp;本项目的非官方平台数据（例如我的收藏列表）来自使用者本地系统或者使用者连接的同步服务，本项目不对这些数据的合法性、准确性负责。</p><br>
        <p><strong>二、版权数据</strong></p>
        <p>2.1&nbsp;使用本项目的过程中可能会产生版权数据。对于这些版权数据，本项目不拥有它们的所有权。为了避免侵权，使用者务必在<strong>24小时内</strong>清除使用本项目的过程中所产生的版权数据。</p><br>
        <p><strong>三、音乐平台别名</strong></p>
        <p>3.1&nbsp;本项目内的官方音乐平台别名为本项目内对官方音乐平台的一个称呼，不包含恶意。如果官方音乐平台觉得不妥，可联系本项目更改或移除。</p><br>
        <p><strong>四、资源使用</strong></p>
        ...
        <p><strong>六、使用限制</strong></p>
        <p>6.1&nbsp;本项目完全免费，且开源发布于&nbsp;<span class="hover underline" @click="openUrl('https://github.com/lyswhut/lx-music-desktop#readme')">GitHub</span>&nbsp;面向全世界人用作对技术的学习交流，本软件不对项目内的技术可能存在违反当地法律法规的行为作保证。</p>
        <p>6.2&nbsp;<strong>禁止在违反当地法律法规的情况下使用本软件</strong>，对于使用者在明知或不知当地法律法规不允许的情况下使用本软件所造成的任何违法违规行为由使用者承担，本软件不承担由此造成的任何直接、间接、特殊、偶然或结果性责任。</p><br>
        <p><strong>七、版权保护</strong></p>
        <p>7.1&nbsp;音乐平台不易，请尊重版权，支持正版。</p><br>
        <p><strong>八、非商业性质</strong></p>
